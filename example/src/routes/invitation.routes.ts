...
router.post("/send", async (req: Request, res: Response) => {
  try {
    ...
    const existingInvitation1 = await InvitationModel.findOne({
      senderId,
      receiverId,
    });
    const existingInvitation2 = await InvitationModel.findOne({
      senderId: receiverId,
      receiverId: senderId,
    });
    ...
    if (existingInvitation1 || existingInvitation2) {
      return res
        .status(400)
        .json({ message: "An invitation already exists between these users." });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error sending invitation.", error });
  }
});
  ...
  try {
    ...
    if (!invitation) {
      return res
        ...
        .json({
          message:
            "Invitation not found or you are not authorized to accept it.",
        });
    }
    ...
    await ContactModel.create({
      userId: receiverId,
      contactId: invitation.data.senderId,
      isFavorite: false,
      createdAt: new Date().toISOString(),
    });
    ...
    res
      .status(500)
      .json({ message: "Server error accepting invitation.", error });
  }
});
...
router.post("/:id/decline", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error declining invitation.", error });
  }
});
